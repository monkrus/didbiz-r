...
import { View, Text, TextInput, Button, Alert, ScrollView, Switch, Image } from "react-native";
...
const defaultVisibility = {
  name: true, title: true, company: true, phone: false, email: true, website: true,
  twitter: false, linkedin: true, github: false, avatarUrl: true, logoUrl: true,
};
...
export default function MyCardScreen() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      ...
      // If local (file:// or app tmp), upload to storage
    ...
    <Controller
      ...
      render={({ field, fieldState }) => (
        <View style={{ marginTop: 12 }}>
          ...
          />
          {fieldState.error && (
            <Text style={{ color: "crimson", marginTop: 4 }}>{fieldState.error.message}</Text>
          )}
        </View>
      )}
    />
...
function VisRow({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
